...
class VehiculosDB:
    ...
    def insertar_posiciones(self, df):
        ...
        fecha = self._to_sql_date(timestamp.date())
        ...
        for _, row in df.iterrows():
            try:
                cursor.execute('''
                MERGE INTO posiciones_vehiculos AS target
                USING (SELECT ? AS timestamp, ? AS vehiculo) AS source
                ON target.timestamp = source.timestamp AND target.vehiculo = source.vehiculo
                WHEN NOT MATCHED THEN
                    INSERT (timestamp, vehiculo, latitud, longitud, velocidad, kilometraje,
                           estado_online, estado_gps, evento, satelites, region, hora_evento,
                           sesion_dia, fecha_registro)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
                ''', (
                    timestamp, row['vehiculo'],
                    timestamp,
                    row['vehiculo'],
                    row['latitud'],
                    row['longitud'],
    ...
    def obtener_posiciones_sesion_actual(self):
        ...
        sesion = self._get_sesion_actual()
        fecha = self._to_sql_date(get_local_now().date())
        ...
        df = pd.read_sql(query, conn, params=(sesion, fecha))
        conn.close()
    ...
    def obtener_todas_posiciones(self, dias=30):
        ...
        fecha_limite = self._to_sql_date((get_local_now() - pd.Timedelta(days=dias)).date())
        ...
        df = pd.read_sql(query, conn, params=(fecha_limite,))
        conn.close()
    ...
    def obtener_recorrido_vehiculo(self, vehiculo, sesion=None, fecha=None):
        ...
        df = pd.read_sql(query, conn, params=(vehiculo, sesion, fecha))
        conn.close()
    ...
    def obtener_alertas_recientes(self, minutos=60, solo_pendientes=True):
        ...
        if solo_pendientes:
            query = '''
            SELECT TOP 50 * FROM alertas_velocidad
        ...
        df = pd.read_sql(query, conn, params=(tiempo_limite,))
        conn.close()
    ...
    def registrar_sesion(self):
        ...
        try:
            ...
            cursor.execute('''
            MERGE INTO sesiones_tracking AS target
            USING (SELECT ? AS fecha, ? AS sesion) AS source
            ON target.fecha = source.fecha AND target.sesion = source.sesion
            WHEN NOT MATCHED THEN
                INSERT (fecha, sesion, inicio_sesion)
                VALUES (?, ?, ?);
            ''', (fecha, sesion, fecha, sesion, timestamp))
    ...
    def asignar_vehiculo_geocerca(self, vehiculo, geocerca_id):
        ...
        try:
            cursor.execute('''
            MERGE INTO vehiculos_geocercas AS target
            USING (SELECT ? AS vehiculo, ? AS geocerca_id) AS source
            ON target.vehiculo = source.vehiculo AND target.geocerca_id = source.geocerca_id
            WHEN MATCHED THEN
                UPDATE SET activa = 1
            WHEN NOT MATCHED THEN
                INSERT (vehiculo, geocerca_id, activa)
                VALUES (?, ?, 1);
            ''', (vehiculo, geocerca_id, vehiculo, geocerca_id))
    ...
    def verificar_geocerca(self, vehiculo, latitud, longitud):
        ...
        cursor.execute('''
        SELECT TOP 1 g.nombre, g.lat_min, g.lat_max, g.lon_min, g.lon_max
        FROM vehiculos_geocercas vg
        INNER JOIN geocercas g ON vg.geocerca_id = g.id
